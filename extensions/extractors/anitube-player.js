...
var Provider = {
    browserScript: function (linkJson) {
        return [
            ...
            "  }",
            "",
            "  // The host keeps this page alive as a relay backend, and the site's own player had to be",
            "  // started only so it would request its manifest. Leaving it playing is both wasted",
            "  // bandwidth and audible: the WebView outlives the screen that asked for it (the host reaps",
            "  // it minutes later, once the relay goes idle), so a page left playing keeps making sound",
            "  // after the user has left the player. Muting - rather than pausing - is deliberate: the",
            "  // site's own playback may be what keeps its session and short-lived URLs fresh for the",
            "  // relay, and only the sound is unwanted.",
            "  function silencePage() {",
            "    var videos = [];",
            "    try { videos = document.querySelectorAll('video'); } catch (error) {}",
            "    for (var i = 0; i < videos.length; i++) {",
            "      var video = videos[i];",
            "      if (!video || video.muted) continue;",
            "      try { video.muted = true; } catch (error) {}",
            "    }",
            "  }",
            "",
