...
var ALLOHA_BROWSER_SCRIPT = [
    ...
    "                if (!url || /^(?:blob|data):/i.test(url) || /cdn\\.plyr\\.io\\/static\\/blank\\.mp4/i.test(url)) return false;",
    "                if (emittedUrls[url]) return false;",
    "                if (/\\.m3u8(?:[?#]|$)/i.test(url)) {",
    ...
    "                if (/\\.mpd(?:[?#]|$)/i.test(url)) {",
    "                    emittedUrls[url] = true;",
    "                    HibikiResolver.stream(url);",
    "                    return true;",
    "                }",
    "                if (/\\.mp4(?:[?#]|$)/i.test(url)) {",
    "                    emittedUrls[url] = true;",
    "                    HibikiResolver.video(url);",
    "                    return true;",
    "                }",
    "                return false;",
    "            }",
    "            function scanValue(raw, depth, seen) {",
    "                if (!raw || depth > 4) return false;",
    "                if (typeof raw === \"string\") return emitUrl(raw);",
    "                if (typeof raw !== \"object\") return false;",
    "                if (seen.indexOf(raw) >= 0) return false;",
    "                seen.push(raw);",
    "                var found = false;",
    "                if (typeof raw.length === \"number\") {",
    "                    for (var i = 0; i < raw.length; i++) found = scanValue(raw[i], depth + 1, seen) || found;",
    "                    return found;",
    "                }",
    "                for (var key in raw) {",
    "                    if (!Object.prototype.hasOwnProperty.call(raw, key)) continue;",
    "                    try { found = scanValue(raw[key], depth + 1, seen) || found; } catch (e) {}",
    "                }",
    "                return found;",
    "            }",
    "            function scanMediaUrls(win, player) {",
    "                var found = false;",
    "                try {",
    "                    var entries = win.performance && win.performance.getEntriesByType(\"resource\");",
    "                    for (var i = 0; entries && i < entries.length; i++) found = emitUrl(entries[i].name) || found;",
    "                } catch (e) {}",
    "                try {",
    "                    var nodes = win.document && win.document.querySelectorAll(\"video[src], video source[src]\");",
    "                    for (var j = 0; nodes && j < nodes.length; j++) {",
    "                        found = emitUrl(nodes[j].currentSrc || nodes[j].src || nodes[j].getAttribute(\"src\")) || found;",
    "                    }",
    "                } catch (e) {}",
    "                try {",
    "                    found = scanValue(player && player.config, 0, []) || found;",
    "                    found = scanValue(player && player.options, 0, []) || found;",
    "                    found = scanValue(player && player.sources, 0, []) || found;",
    "                } catch (e) {}",
    "                return found;",
    "            }",
    ...
    "                var noQualityPolls = 0;",
    "                var noQualityTimer = setInterval(function(){",
    "                    if (scanMediaUrls(win, player) || ++noQualityPolls >= 30) {",
    "                        clearInterval(noQualityTimer);",
    "                        HibikiResolver.done();",
    "                    }",
    "                }, 250);",
    "                return \"starting\";",
    ...
    "                HibikiResolver.quality(quality + \"p\");",
    "                try {",
