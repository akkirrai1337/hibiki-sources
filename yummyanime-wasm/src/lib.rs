...
fn execute(request: RuntimeRequest) -> Result<Value, String> {
    ...
    if matches!(&request.operation, RuntimeOperation::Search) { validate_year_range(&request.payload, "YummyAnime")?; }
