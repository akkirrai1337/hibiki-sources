...
fn card_titles_dom(html: &str) -> Result<Vec<Value>, String> {
    ...
    for (index, item) in parsed.iter().enumerate() {
        validate_title_metadata(item, "AnimeGo", &format!("catalog item {index}"))?;
    }
    Ok(parsed)
}
...
fn execute(request: RuntimeRequest) -> Result<Value, String> {
    if matches!(&request.operation, RuntimeOperation::Search | RuntimeOperation::Latest) { validate_pagination(&request.payload, "AnimeGo")?; }
    if matches!(&request.operation, RuntimeOperation::Search) { validate_search_query(&request.payload, "AnimeGo")?; }
    if matches!(&request.operation, RuntimeOperation::Search) { validate_string_filters(&request.payload, &["typeAliases", "statusAliases", "includedGenreAliases", "excludedGenreAliases"], "AnimeGo")?; }
    match request.operation {
        ...
        RuntimeOperation::PlaybackGroups => {
            ...
            let payload = json!({ "groups": if episodes.is_empty() { Vec::<Value>::new() } else { vec![json!({ "id": id, "title": "AnimeGo", "qualityLabel": null, "episodes": episodes })] } });
            validate_playback_payload(&payload, "AnimeGo")?;
            Ok(payload)
        }
        RuntimeOperation::PlayerLinks => {
            ...
            validate_player_links_payload(&payload, "AnimeGo")?;
            Ok(payload)
        }
    }
}
