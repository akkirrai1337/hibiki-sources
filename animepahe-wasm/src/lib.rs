...
fn execute(request: Request) -> Result<Value, String> {
    ...
    if matches!(&request.operation, Operation::Search) { validate_search_query(&request.payload, "AnimePahe")?; }
    match request.operation {
        ...
        Operation::Details => { let id = request.payload.get("id").and_then(Value::as_str).ok_or("AnimePahe details id is missing")?; let id = safe_path_segment(id).ok_or("AnimePahe details id is invalid")?; let parsed = details(&page(&request.request_id, &format!("/anime/{id}"))?, id)?; validate_title_metadata(&parsed, "AnimePahe", "details")?; validate_title_identity(&parsed, id, "AnimePahe")?; Ok(parsed) }
